...
For this project, I have designed a realistic virtual piano. The screen is an entire keyboard of a piano which consists of 7 keys: C, D, E, F, G, A, B (from left to right). Each lettered key plays its corresponding note when clicked on by the user, so, the pitch of the note evidently gets higher from left to right. The user's cursor also contains a black dot that follows it around and decreases in size from left to right. When the user clicks a key, this dot changes colour quickly from black to gray as a way to let the user know that they have successfully clicked. In addition, each key's letter is displayed near the top of each key to help the user understand which key is which, especially for those who are unfamiliar with pianos. And, similar to the cursor dot, the font size of the letters displayed gets smaller in size from bottom to top. So, for example, if the user's cursor is in the bottom left corner of the screen than the letter's font size is large and the dot is large, but if the user's cursor is in the top right corner of the screen than the letter's font size is small and the dot is small. Finally, to make the piano more interactive for the user, there are 6 different  keyboard colours to choose from (including the default white). The 6 colours are: white(default), red, green, blue, yellow, and orange. To select a colour, the user must simply press the key on their computer keyboard that corresponds to the first letter of that colour, so for example, the user would click 'y' to turn the piano yellow and so on. Along with this, when the piano is a colour OTHER THAN white, than the shade of the colour gradually changes from dark to light when the piano keys are clicked from left to right. So if it is red, than clicking the first key (C note) will make the piano have a dark shade of red, but clicking the last key (B note) will make the piano have a light shade of red (the higher the pitch, the lighter the shade). Overall, my synaesthetic instrument allows one to experience playing the piano without actually playing one, but to also enjoy the visual aspects of it while creating some music!
